...
gg_richness <- plot_richness(ryan_rare_new, x = "Condition", 
...
plot.pd <- ggplot(sample_data(ryan_rare_new), aes(x= Condition, y = PD, color = Condition)) + 
  geom_boxplot() +
  xlab("Condition") +
  ylab("Phylogenetic Diversity")

ggsave(filename = "plot_pd_new.png"
       , plot.pd
       , height=4, width=6)