...
# Healthy-only taxa table
healthy_only_taxa <- isa_CD_Healthy_labeled %>%
  filter(group_label == "Healthy_only")
View(healthy_only_taxa)
...
isa_CD_Healthy <- isa_table_all %>%
  filter(`s.Ulcerative Colitis` == 0)
isa_CD_Healthy <- isa_CD_Healthy %>%
  select(-`s.Ulcerative Colitis`)
View(isa_CD_Healthy)
