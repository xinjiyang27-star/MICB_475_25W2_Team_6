...
UC_stat <- subset_samples(ryan_mpt_RA, Condition=="Ulcerative Colitis")
...
CD_ASVs <- core_members(CD_stat, detection=0, prevalence = 0.7)
UC_ASVs <- core_members(UC_stat, detection=0, prevalence = 0.7)
healthy_ASVs <- core_members(healthy_stat, detection=0, prevalence = 0.7)

# Make a Venn-diagram
condition_venn <- ggVennDiagram(x=list(Crohns_Disease = CD_ASVs, Ulcerative_Colitis = UC_ASVs, Healthy = healthy_ASVs))

# Make sure that you have a line that saves the Venn diagram as a png and this file is present within your project folder
ggsave("Ryan_Venn_Condition.png", condition_venn)
...
mostabundantUC<-tax_table(prune_taxa(UC_ASVs,ryan_mpt))
...
write.csv(mostabundantUC, "most_abundant_taxa_UC.csv", row.names = FALSE)
