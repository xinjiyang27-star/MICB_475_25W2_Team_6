...
tax_mat["71cdfae8e8fa2e8918f5db97f0fe52ab", "Genus"] <- "g__Azovibrio"
tax_mat["7f83ae4b4bd781e53f9f26bf3e6c5680", "Genus"] <- "g__Senimuribacter"
...
tax_table(ps)["71cdfae8e8fa2e8918f5db97f0fe52ab", ]

# Aggregate reads to Genus level
ps <- tax_glom(ps,"Genus")

# Load the indicator taxa results
filtered_data<- read.csv("ISA_onlyCD.csv")
...
filtered_data_with_genus <- filtered_data %>%
  mutate(Genus = case_when(
    ASV == "71cdfae8e8fa2e8918f5db97f0fe52ab" ~ "g__Azovibrio",
    ASV == "7f83ae4b4bd781e53f9f26bf3e6c5680" ~ "g__Senimuribacter",
    TRUE ~ Genus
  ))
...
temp = ps_clr %>% 
  subset_taxa(Genus=='g__Anaerococcus') %>% 
  psmelt()
...
ggsave("Anaerococcus_abundance.png", box_plot_RF, width = 8, height = 6, dpi = 300)