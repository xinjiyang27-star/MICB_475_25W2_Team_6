...
ps = readRDS("ryan_filt.rds")

# Convert taxonomy table to matrix and assign genus to your ASV
tax_mat <- as(tax_table(ps), "matrix")
tax_mat["35d3f5184c7c7dd9f2b01f01c9b4b71a", "Genus"] <- "g__Romboutsia"
tax_mat["949d11292146e12904547a6db61ce024", "Genus"] <- "g__Anaerosacchariphilus"
...
tax_table(ps)["35d3f5184c7c7dd9f2b01f01c9b4b71a", ]

# Aggregate reads to Genus level
ps <- tax_glom(ps,"Genus")

# Load the indicator taxa results
ISA_results<- read.csv("ISA_significant_taxa.csv")
...
filtered_data_with_genus <- filtered_data %>%
  mutate(Genus = case_when(
    ASV == "949d11292146e12904547a6db61ce024" ~ "g__Anaerosacchariphilus",
    ASV == "35d3f5184c7c7dd9f2b01f01c9b4b71a" ~ "g__Romboutsia",
    TRUE ~ Genus
  ))
...
temp = ps_clr %>% 
  subset_taxa(Genus=='g__Anaerosacchariphilus') %>% 
  psmelt()
